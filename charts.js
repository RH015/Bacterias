...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var barData = [{
      ...
      y: yticks.slice(0,10).reverse().map(id => "OUT " + String(id)),
    ...
    var filter_wfreq = data.metadata.filter(sampleObj => sampleObj.id == sample);
    var wfreq = parseFloat(filter_wfreq[0].wfreq);
    console.log (wfreq)
    var gaugeData = [{
      mode: "gauge+number",
      type: "indicator",
